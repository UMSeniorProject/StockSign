...
public class ConnectToKinveyTask implements ActivityConstants {
	...
	public boolean testKinveyService() {
		// call `myClient.user().logout().execute() first -or- check
		// `myClient.user().isUserLoggedIn()` before attempting to login again
		...
		if (login) {
			...
			mKinveyClient.user().login(new KinveyUserCallback() {
				...
				public void onFailure(Throwable error) {
					Log.e("KinveyLogin", "Login Failure: "
							+ error.getCause().getMessage(), error);
					conn_success = false;
				}
				...
				public void onSuccess(User result) {
					Log.i("KinveyLogin",
							"Logged in successfully as " + result.getId());
					mKinveyClient.ping(new KinveyPingCallback() {
						@Override
						public void onSuccess(Boolean result) {
							Toast.makeText(
									callingActivity.getApplicationContext(),
									"kinvey ping success!", Toast.LENGTH_LONG)
									.show();
							conn_success = true;
						}
						...
						public void onFailure(Throwable error) {
							Toast.makeText(
									callingActivity.getApplicationContext(),
									"kinvey ping failed!", Toast.LENGTH_LONG)
									.show();
							conn_success = false;
						}
					});
